...
/*
    ...
    */
    ...
    // insert application
    await db.execute(`INSERT INTO WalkRequests (request_id, walker_id, applied_at, status) VALUES 6, 2, '2025-06-08 08:00:00', 'accepted';`);
    await db.execute(`INSERT INTO WalkRequests (request_id, walker_id, applied_at, status) VALUES 7, 2, '2025-06-08 08:00:00', 'accepted';`);
...
app.get('/api/walkers/summary', async (req, res) => {
    try {
        const [open] = await db.execute(
            `SELECT u.username AS walker_username, COUNT(r.rating_id) AS total_ratings, TRUNCATE(AVG(r.rating), 1) AS average_rating, COUNT(DISTINCT CASE WHEN wr.status = 'completed' AND wa.status = 'accepted' THEN wr.request_id ELSE NULL END) AS completed_walks
